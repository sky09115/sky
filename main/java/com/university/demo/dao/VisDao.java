...
public interface VisDao extends BaseMapper<User> {
    ...
    @Select("  select count(distinct line_code) from tb_flow ")
    Integer dash02();
    ...
    @Select(" select m.name, f.flow as value from  tb_flow f, tb_metro m where m.code = f.site_code and xl ='是' order by flow desc limit 100;  ")
